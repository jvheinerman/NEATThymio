...
class ForagingTask(TaskEvaluator):
    ...
    def prox_readings_ok_call(self, proxvalues):
        self.proxvalues = np.array([(proxvalues[0] + proxvalues[4])/2, (proxvalues[5] + proxvalues[6])/2] ,dtype='f')
        self.proxvalues = [(float(x) - float(pr.SENSOR_MAX[0]/2))/float(pr.SENSOR_MAX[0]/2) for x in proxvalues]
        print "prox values: ", self.proxvalues
